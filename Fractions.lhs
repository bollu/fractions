= An exposition of `kmett/fractions`: how to compute pi to 100 places fast:

<img src="docs/farey-diagram.png"/>
<img src="docs/mediant-rule.png"/>
<img src="docs/semicircle-convergence-square-root-2.png"/>
<img src="docs/seven-by-sixteen-farey.png"/>
...
An integral domain is a ring which has no zero divisors. That is,
we not have numbers `a, b != 0` such that `ab = 0.` Intuitively, if we have
`ab = 0`, then we can say that `a` divides `0`. An integral domain is
a ring where such an `a` [and such a `b`] does not exist.
...
>--------------------------------------------------------------------------------
>
>data P a = P [a] -- p !! i = coefficient of (x^i)
>  deriving (Show, Eq, Foldable, Functor)


`zeroes` appends `n` zeroes.
